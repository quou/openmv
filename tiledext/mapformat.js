	...
	if (layer.isTileLayer) {
		...
		for (var ii = 0; ii < layer.objectCount; ii++) {
			...
			if (layer.name === "transition_triggers" || layer.name == "doors") {
				...
				fwrite_float(obj.property("intensity"), file);
				fwrite_float(obj.property("range"), file);
...
var dat_format = {
	...
	write:function(map, filename) {
		...
		if (map_name == undefined || map_name == null) {
			...
			fwrite_string(map_name, file);
		}

		var is_dark = map.property("dark");
		if (is_dark == undefined || is_dark == null) {
			fwrite_uint(0, file);
		} else {
			fwrite_uint(is_dark, file);
		}
